...
==To test out the DDDS, we will send it a test-script, and watch the output on a spectrum analyzer with START freq 0 MHz, and STOP freq 40 MHz. Connect OUT1 of the RP to your spectrum analyzer, and in the shell (in the directory of the unzipped files), run (with rpIP replaced with the actual IP address, as above):
"python DDDS_Test.py RP_IP=rpIP SOFTWARETRIGGER=1"

This should induce the RP to output a frequencies ramp for each of channels 1 and 2, starting and 1 MHz and stopping at 30MHz for channel 1, and reverse for channel 2, over 5 seconds.
